 ...
 * @author litfb
 * @version 1.3
 * @apiNote 此代码示例仅限于企业自建应用/企业自有业务系统推送消息到友空间的开发者
 */
public class EnterpriseMessageDemo {

    private static final Logger logger = LoggerFactory.getLogger(EnterpriseMessageDemo.class);

    // #1 获取AccessToken
    /**
     * 获取AccessToken
     */
    private static final String URL_ACCESS_TOKEN = "https://openapi.yonyoucloud.com/token";
     ...
     * #1 获取AccessToken
     *
     * @param openAppId  鉴权参数，在友空间后台应用管理查看
     * @param openSecret 鉴权参数，在友空间后台应用管理查看
     * @return access_token
     */
    public static String getAccessToken(String openAppId, String openSecret) {
        // params
        Map<String, String> params = new HashMap<>();
        params.put("appid", openAppId);
        params.put("secret", openSecret);
        // request result
        String result = HttpClientUtils.doGet(URL_ACCESS_TOKEN, params);
        // JSONObject
    ...
    private static boolean checkResult(String result, String identify) {
        ...
        if (result != null && !"".equals(result)) {
            ...
            if (responseData != null) {
                ...
                logger.info("response message: " + responseData.getString("msg"));
                if (flag != null && flag == 0) {
